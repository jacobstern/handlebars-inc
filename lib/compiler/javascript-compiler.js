...
JavaScriptCompiler.prototype = {
  ...
  /**
   * Since the code unit of `handlebars-idom` is an unevaluated function,
   * generate a void function call for the expression at hand.
   */
  appendToBuffer: function(source, location) {
  ...
  createFunctionContext: function(asObject) {
    ...
    if (asObject) {
      params.push(thunk);
      return Function.apply(this, params); // TODO: What does this actually do?
    } else {
  ...
  //
  // Invoke an incremental-dom thunk if it is not null or undefined, otherwise
  // push `container.noop()`.
  append: function() {
    logOpCode('append', arguments);

    if (this.isInline()) {
      this.replaceStack((current) => [' != null ? ', current, ' : container.noop']);
