...
import {isArray} from '../vendor/handlebars/utils';
import CodeGen from '../vendor/handlebars/compiler/code-gen';
import { parseFragment } from '../core/parser';
...
JavaScriptCompiler.prototype = {
  ...
  compilerInfo: function() {
    ...
    return [revision, versions, { idom: this.idom} ];
  },
  ...
  compile: function(environment, options, context, asObject) {
    this.idom = Boolean(options.idom); // Possibly temporary, flag to opt into new behavior
    ...
    if (!this.decorators.isEmpty()) {
      // FIXME: Ignoring this code path for now
  ...
  generateIncrementalDOMCall: function (method, args) {
    return this.source.functionCall(`container.IncrementalDOM.${method}`, '', args) + ';';
  },

  generatePropertyValuePair: function (valuePair) {
    return this.source.generateArray(valuePair.map(entry => this.quotedString(entry)));
  },

  generateCallForDOMOperation: function (op) {
    switch (op.type) {
      case 'text': {
        return this.generateIncrementalDOMCall(
          'text',
          [
      ...
      case 'elementOpen': {
        return this.generateIncrementalDOMCall(
          ...
          [
            ...
            this.quotedString(shortid.generate()),
            op.value.propertyValuePairs.map(pair =>this.generatePropertyValuePair(pair))
          ]);
      }
      case 'elementClose': {
        return this.generateIncrementalDOMCall(
  ...
  createFunctionContext: function(asObject) {
    ...
    if (asObject) {
      ...
      let body = source;
      if (this.idom) {
        // Add thunk for incremental-dom
        body = this.source.wrap(['return function() {\n', body, '}']);
      }
  ...
  appendEscaped: function() {
    ...
    if (this.idom) {
      this.pushSource(this.generateIncrementalDOMCall(
        'text',
        [this.source.wrap(
          [
