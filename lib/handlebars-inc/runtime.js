...
import { parseAttributes } from './core/attributes-parser';
...
export function template(templateSpec, env) {
  ...
  function invokePartialWrapper(partial, context, options) {
    ...
    options = { ...options, backend: BACKEND_IDOM, idom: this.idom, noEscape: this.noEscape };
  ...
  let container = {
    buildDynamicOpeningTag: function(name, key, contents) {
      const keyValuePairs = parseAttributes(contents);
      const dynamicAttrs = [];
    ...
    runIdomToText: runIdomToText,

    escapeExpression: function(thunkOrText) {
      if (typeof thunkOrText === 'function') {
        return thunkOrText; // Escaping is taken care of with incremental-dom calls
      } else {
        if (this.noEscape) {
          return thunkOrText;
        }
        return Utils.escapeExpression(thunkOrText);
      }
    },
