...
JavaScriptCompiler.prototype = {
  ...
  compile: function(environment, options, context, asObject) {
    ...
    if (!this.decorators.isEmpty()) {
      throw new Error('Decorators are not implemented for handlebars-inc');
    } else {
  ...
  generateStatics: function(valuePairs) {
    ...
    return this.source.generateArray(
      flattened.map(entry => this.quotedString(entry))
    );
  },
  ...
  /**
   * If `data-idom-key` is specified as a static attribute in
   * `propertyValuePairs`, return the value of that attribute. Otherwise,
   * generate a random key.
   */
  getIdomKey(propertyValuePairs) {
    for (let [key, value] of propertyValuePairs) {
  ...
  generateCallForDOMOperation: function(op) {
    switch (op.type) {
      case 'text': {
        return this.generateIdomCall('text', [
          this.quotedString(op.value.text),
        ]);
      }
      case 'emptyElement': {
        const key = this.getIdomKey(op.value.propertyValuePairs);
        return this.generateIdomCall('elementVoid', [
      ...
      case 'elementOpen': {
        const key = this.getIdomKey(op.value.propertyValuePairs);
        return this.generateIdomCall('elementOpen', [
          this.quotedString(op.value.tagName),
          this.quotedString(key),
          this.generateStatics(op.value.propertyValuePairs),
        ]);
      ...
      case 'elementClose': {
        return this.generateIdomCall('elementClose', [
          this.quotedString(op.value.tagName),
        ]);
      }
    }
  ...
  initPartialTagClosure: function(content) {
    ...
    this.source.push(
      'var c = container.extend({}, container, {idom: idom, noEscape: true});\n'
    );
    if (content) {
      this.source.push(
        this.generateIdomCall('text', [this.quotedString(content)])
      );
    }
  },
  ...
  finalizePartialTag: function(tagName) {
    this.source.push('});\n');
    this.source.push(
      this.source.functionCall('container.buildDynamicOpeningTag', null, [
    ...
    if (isEmptyElement(tagName)) {
      this.source.push(
        this.generateIdomCall('elementClose', [this.quotedString(tagName)])
      );
    }
    this.source.push('})();\n');
  },
  ...
  appendParseFallbackText: function(text) {
    let append = this.source.functionCall('c.appendResult', null, [
  ...
  appendEscaped: function() {
    ...
    this.pushSource(
      this.source.functionCall('c.appendResult', null, [
        this.source.functionCall('c.escapeExpression', null, [
          this.popStack(),
        ])
      ])
    );
  ...
  invokeKnownHelper: function(paramSize, name) {
    ...
    let helperCall = this.source.functionCall(
      helper.name,
      'call',
      helper.callParams
    );
    this.push(helperCall);
  },
  ...
  setupParams: function(helper, paramSize, params) {
    ...
    // Expose unique key per helper call
    options.uniqueKey = this.quotedString(this.getGeneratedId());

    // Helpers also have access to the current incremental-dom instance
    options.idom = 'c.idom';
